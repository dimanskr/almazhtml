...
function woo_variations_table_available_options_btn(){
  global $product;
  	if(!$product->is_type('variable'))
  		return;
	// отображать если пользователь зарегистрирован
...
// отображение вариативной таблицы после продукта after product summary
add_filter('woocommerce_after_single_product_summary','variations_table_print_table',10);
// отображение вариативной таблицы продукта в кратком описании товара
// add_filter('woocommerce_single_product_summary','variations_table_print_table',50);
              ...
              <table class="variations">
                <thead>
                  <tr>
                    ...
                    <th class="quantity"><?php echo __("Количество", 'woo-variations-table'); ?></th>
                ...
                <tbody>
                  <tr v-for="(entry, index) in filteredData" :class="'variation-'+entry.variation_id+ ' image-'+ imageClass(entry['image_link'])">
                    ...
                    <td class="add-to-cart"><button :ref="'variation-'+entry.variation_id" @click="addToCart(entry)" type="submit" class="single_add_to_cart_button button alt" :class="{added: entry.added}"><?php echo __("Заказать", 'woo-variations-table'); ?></button></td>
                  </tr>
                </tbody>
              </table>
            </script>
            ...
            <!--        <div class="filter">
                    ...
                    <div v-for="(attribute, key, index) in attributes" class="filter">
                        ...
                        <select v-model="activeFilters[index]" @change="setFilters()" class="form-control">
                          <option value=""><?php echo __('Все', 'woo-variations-table'); ?></option>
                ...
                var vm = new Vue({
                  ...
                  data: {
                    ...
                    gridColumns: [
                        ...
                        {key: 'weight_html', title: 'Вес', type: 'text'},
