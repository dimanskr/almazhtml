...
function vt_woocommerce_variable_add_to_cart($allsets)
{
    ...
    <table
      id="tb_' . $vtrand . '"
      class="table vartable product_variations_table ' . ($sorting_js == 1 ? 'is_sortable' : '') . ' ' . $vartable_table_class . '"
      data-random="' . $vtrand . '"
        ...
        if ($vartable_thumb == 1) {
            ...
            if (isset($rowimg[0])) {
                $allcolumns['vartable_thumb'] = '<td class="thumbcol"  data-label="' . apply_filters('vartable_dl_thumb', $headenames['vartable_thumb'], $value) . '">
                  <a href="' . $rowimgfull[0] . '" itemprop="image" class="variationimg zoom ' . apply_filters('vartable_thumb_class_filter', 'thumb', $value) . '" title="' . $product->get_title() . ' - ' . implode(' - ', $value['attributes']) . '"  data-rel="prettyPhoto">
...
// get attribute names
        ...
        if ($vartable_weight == 1) {
            ...
            if ($product_variation->has_weight()) {
              ...
              <td class="weight_col" data-sort-value="' . $product_variation->get_weight() . '" data-label="' . apply_filters('vartable_dl_weight', $headenames['vartable_weight'], $value) . '">' . $product_variation->get_weight() . ($product_variation->has_weight() ? ' ' . 'г' : '') . '</td>';
                $anyweight = 1;
            } else {
                $allcolumns['vartable_weight'] = '
