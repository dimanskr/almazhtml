    ...
    class WCB_Category_Banner
{
        ...
        public function __construct()
    {
            ...
            add_action('woocommerce_archive_description', array($this, 'wcb_show_category_banner'), 5);
        ...
        public function wcb_product_cat_taxonomy_custom_fields($tag)
    {
			...
			<tr class="form-field banner_url_form_field">
			    ...
			    <td>
			    	<fieldset>
						...
						<label class='banner_url_label' ><?php if ($banner_id != null) {
                echo basename(wp_get_attachment_url($banner_id));
            }
            ?></label>
			    	</fieldset>

			    	<fieldset>
						<img class="cat_banner_img_admin" src="<?php if ($banner_id != null) {
                echo wp_get_attachment_url($banner_id);
            }
            ?>" />
			    	</fieldset>

					<input type="hidden" class='wcb_image' name='term_meta[banner_url_id]' value='<?php if ($banner_id != null) {
                echo $banner_id;
            }
            ?>' />
				</td>
			...
			<tr class="form-field banner_link_form_field">
			    ...
			    <td>
			    	<fieldset>
						<input type="url" name='term_meta[banner_link]' value='<?php if ($banner_link != null) {
                echo $banner_link;
            }
            ?>' />
			...
			<tr class="form-field auto_display_banner">
			    ...
			    <td>
			    	<fieldset>
			    	<input name="term_meta[auto_display_banner]" type="checkbox" value="on" class="auto_display_banner" <?php if ($auto_display_banner) {
                echo " checked ";
            }
            ?>/>
        ...
        public function wcb_show_category_banner()
    {
            ...
            if (is_product_category()) {
                ...
                if ((isset($term_options['auto_display_banner']) && $term_options['auto_display_banner'] == 'on') || !isset($term_options['auto_display_banner'])) {
                    ...
                    if ($url != false) {?>
					<div class="category-banner" style="background-image: url('<?php echo $url; ?> ')">
