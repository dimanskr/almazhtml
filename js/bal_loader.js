...
function button_autopager(callback) {
    ...
    if (d_end != d_total) {
        $('[data-bal="bal-div"]').before('<div class="row bal" style="margin-bottom: 10px;"><div class="col-sm-12"><div class="bal-box-next"><a href="' + next_url + location.hash + '" rel="next">' + text_show_next.replace(/{show}/gi, e_length_s).replace(/{items}/gi, declension(e_length_s)) + '</a></div><span class="results">' + text_show_all.replace(/{show}/gi, e_length).replace(/{items}/gi, declension(e_length)).replace(/{shows}/gi, e_total_p) + '</span></div>');
    } else {
    ...
    $.autopager({
        ...
        start: function (current, next) {
            ...
            $('[data-bal="bal-div"]').after('<div class="bal-loading"><div class="bal-center"><img alt="' + text_loading + '" src="image/three-dots.svg" /></div></div>');
